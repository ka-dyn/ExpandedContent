...
namespace ExpandedContent.Tweaks.TheElderMythos {
    internal class YogSothoth {
        ...
        public static void AddYogSothothFeature() {
            ...
            var YogSothothIcon = AssetLoader.LoadInternal("Deities", "Icon_YogSothoth.jpg");
            var YogSothothFeature = Helpers.CreateBlueprint<BlueprintFeature>("YogSothothFeature", (bp => {

                bp.SetName("Yog-Sothoth");
                bp.SetDescription("\nTitles: Lurker at the Threshold, The Key and the Gate, The Watcher   " +
                    "\nAlignment: Chaotic Neutral   " +
                    "\nAreas of Concern: Gates, Space, Time   " +
                    "\nRank: Outer God   " +
                    "\nDomains: Chaos, Darkness, Knowledge, Travel, Void (Magic)   " +
                    "\nSubdomains: Dark Tapestry, Exploration, Memory, Night, Stars, Thought   " +
                    "\nFavoured Weapon: Dagger   " +
                    "\nHoly Symbol: Black spiral   " +
                    "\nAccording to some, the Dark Tapestry is not so much a location as a thing that connects all places and possibilities. These texts go on to suggest that this it is conscious—that " +
                    "it is aware. Yog-Sothoth is normally said to dwell within the Dark Tapestry, but some ancient legends say he is the Dark Tapestry. Certainly he has great power over time and space, " +
                    "and can exist at multiple times in multiple realities. Distance has no meaning to him, yet he seems unable to properly exist in the reality shared by most mortal life. He can only " +
                    "partially intrude upon this dimension, taking on the shape of a slithering mass of iridescent spheres or glowing motes of light when he does. Or perhaps Yog-Sothoth can exist in " +
                    "this world but has no reason to do so, and what mortals see and tremble before is but his shadow. Yog-Sothoth’s cultists believe he is slowly preparing the many worlds of the " +
                    "universe for a new age presaged by the wakening of the Great Old Ones, during which time the inhabitants of these worlds will be wiped out to make way for the true masters. Cults " +
                    "often call upon Yog-Sothoth to bless unborn children, turning them into monstrous hybrids that help to prepare the unsuspecting world for the time when the stars themselves " +
                    "shall be right for the Great Old Ones to rule once again.");
                bp.m_Icon = YogSothothIcon;
                bp.Ranks = 1;
            ...
            DeityTools.LazySacredWeaponMaker("YogSothoth", YogSothothFeature, WeaponCategory.Dagger);
