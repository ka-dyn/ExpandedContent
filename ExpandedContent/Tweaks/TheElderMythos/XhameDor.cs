...
namespace ExpandedContent.Tweaks.TheElderMythos {
    internal class XhameDor {
        ...
        private static readonly BlueprintFeature UndeadDomainFeature = Resources.GetModBlueprint<BlueprintFeature>("UndeadDomainFeature");
        ...
        public static void AddXhameDorFeature() {
            ...
            var XhameDorIcon = AssetLoader.LoadInternal("Deities", "Icon_XhameDor.jpg");
            var XhameDorFeature = Helpers.CreateBlueprint<BlueprintFeature>("XhameDorFeature", (bp => {

                bp.SetName("Xhamen-Dor");
                bp.SetDescription("\nTitles: The Inmost Blot, The Star Seed   " +
                    ...
                    "\nHoly Symbol: Sphere of tendrils   " +
                    "\nThe sentient infestation known as Xhamen-Dor, first cultivated in the sewers of the alien city of Carcosa, infected and transformed countless worlds before some ancient " +
                    "catastrophe or miscalculation nearly destroyed it. Reduced to a single blot of fungoid sentience trapped within a comet, the Great Old One eventually came to Golarion during " +
                    "Earthfall, accidentally plucked from the sky just as the Starstone was in the aboleths’ assault upon the doomed nation of Azlant. Where Xhamen-Dor’s comet came to rest is " +
                    "unknown, but its cult agrees that the fallen star that bears the Great Old One’s core lies in the depths of a large but remote lake somewhere in the world. Since that date, " +
                    "Xhamen-Dor has slowly begun wakening, growing and sending filaments of its fungoid body throughout the world to rise up here and there to infest and spread. Those infected " +
                    "by the Inmost Blot first become sickly, then begin to simultaneously decay and transform, eventually becoming undead monsters animated by a combination of necromantic energy " +
                    "and the alien fungal masses that writhe in their cores. Cultists of Xhamen-Dor often take these parasitic creatures into themselves under the belief that they will live on " +
                    "as part of the creatures that emerge from their bodies, but just as often use unsuspecting sacrifices to serve as hosts. Foul Xhamen-Dor is said to have once resembled a " +
                    "tangled mass of fungoid tendrils growing from the shattered bones of some ancient reptilian lifeform, though what nightmarish shape the slowly growing blight has taken on " +
                    "today is unknown.");
                bp.m_Icon = XhameDorIcon;
                bp.Ranks = 1;
                ...
                bp.AddComponent<AddFacts>(c => {
                    c.m_Facts = new BlueprintUnitFactReference[1] { UndeadDomainFeature.ToReference<BlueprintUnitFactReference>() };
                });
